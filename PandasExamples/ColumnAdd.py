...
#> ColumnAdd --newColumnNames NewEmptyColumn --exampleTitle Add a New Empty Column --example Add an empty column to a dataframe that has a desired column name. Specify the desired column name with the newColumnNames option. Each row will have missing values in the new column. 
...
#> ColumnAdd 1 --newColumnNames Columnof1s --exampleTitle Added a Column With Set Value --example Add a column to the dataframe where every row in the column has the same value. In this case, the column name is Columnof1s and the value of each row in the column will be 1 
