...
#> Histogram --x High --exampleImage Histogram1.png --exampleTitle Plot Histogram of Single Column --example A histogram is useful to see how values are distributed across a column. In this example, we plot the histogram of the High column to observe the distribution of daily high prices in the Apple stock dataset. 
...
#> Histogram --x HighDivLow --exampleImage Histogram2.png --exampleTitle Plot Histogram of Computed Ratio Column --example You can plot histograms of computed columns the same way as standard columns. Here we first compute the HighDivLow ratio by dividing the High column by the Low column, then plot a histogram to analyze how this ratio varies across the dataset. 
...
#> Histogram --x Attack Sp.Atk Defense Sp.Def --bins 20 --exampleImage Histogram5.png --exampleTitle Compare Distributions of Multiple Columns --example When comparing the distributions of several related columns, you can overlay multiple histograms on the same plot. In this case, we compare Attack, Sp.Atk, Defense, and Sp.Def from the Pokemon dataset, using the same bin count to observe their relative distributions.
