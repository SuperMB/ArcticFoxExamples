...
#> Bubble Points --size Wins --exampleImage Bubble3.png --exampleTitle Bubble Plot by Index --example When visualizing data without a natural x-axis, plotting against index can still provide insight. In this example, we use the index as the x-axis and plot Points as the y-axis, using bubble size to represent the number of Wins per player.
...
#> Bubble --x Wins --y BLK AST FG_Percent --size Losses --exampleImage Bubble4.png --exampleTitle Multi-Series Bubble Plots --example This chart overlays three separate bubble plots to show how blocks, assists, and field goal percentage vary with wins. Each metric is plotted against wins, and the bubble size corresponds to the number of losses, allowing for comparative visual analysis of multiple performance indicators.
